...
std::pair<Replxx::hints_t, Replxx::hints_t> hook_hint_with_help(std::string const& context, int& contextLen, Replxx::Color& color,
		std::vector<std::string> const& user_data, std::vector<std::string> const& user_help, bool);
...
std::pair<Replxx::hints_t, Replxx::hints_t> hook_hint_with_help(std::string const& context, int& contextLen, Replxx::Color& color, std::vector<std::string> const& examples, std::vector<std::string> const& exhelp, bool ignoreCase)
{
	Replxx::hints_t hints;
	Replxx::hints_t helps;
	...
	if (prefix.size() >= 2 || (! prefix.empty() && prefix.at(0) == '.')) {
		...
		for (int i = 0; i < examples.size(); ++i) {
			if ( eq( examples[i], prefix, prefix.size(), ignoreCase && lowerCasePrefix ) ) {
				hints.emplace_back(examples[i].c_str());
				helps.emplace_back(exhelp[i].c_str());
			}
		}
	}
	...
	return std::make_pair(hints, helps);
}
...
int main( int argc_, char** argv_ ) {
	...
	std::vector<std::string> examplesHelp;
	examplesHelp.reserve(examples.size());
	for (const auto & ex: examples)
		examplesHelp.emplace_back(std::string("Help:\n  first line\n  second line\n  for ").append(ex));
	...
	rx.set_with_help(true);
	rx.set_hint_with_help_callback( std::bind( &hook_hint_with_help, _1, _2, _3, cref( examples ), cref( examplesHelp ), ignoreCase ) );
	if ( promptInCallback ) {
	...
	rx.bind_key_internal( Replxx::KEY::control( Replxx::KEY::UP ),     "hint_previous_with_help" );
	rx.bind_key_internal( Replxx::KEY::control( Replxx::KEY::DOWN ),   "hint_next_with_help" );
